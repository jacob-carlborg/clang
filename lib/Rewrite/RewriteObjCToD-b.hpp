...
namespace {
  ...
  public:
    ...
    RewriteObjCToD(std::string inFile, llvm::raw_ostream *OS,
    ...
    // Rewriting.
	std::string rewriteFunctionPointerType (const QualType& type);
	std::string rewriteObjCObjectPointerType (const QualType& type);
	std::string rewriteType (const QualType& type, bool rewriteIdToObject = true);
	std::string rewriteType (std::string str);
	std::string rewriteIdentifier (std::string str);
    ...
    void RewriteObjCToDMethodDecl(ObjCMethodDecl *MDecl, std::string &ResultStr);
    ...
    void RewriteMethodDeclaration(ObjCMethodDecl *Method, size_t length = 0);
    void RewriteProperty(ObjCPropertyDecl *prop);
    void RewriteFunctionDecl(FunctionDecl *FD);
    void RewriteBlockPointerType(std::string& Str, QualType Type);
    void RewriteBlockPointerTypeVariable(std::string& Str, ValueDecl *VD);
    void RewriteBlockLiteralFunctionDecl(FunctionDecl *FD);
    void RewriteObjCToDQualifiedInterfaceTypes(Decl *Dcl);
    void RewriteTypeOfDecl(VarDecl *VD);
    void RewriteObjCToDQualifiedInterfaceTypes(Expr *E);
    ...
    Stmt *RewriteObjCToDIvarRefExpr(ObjCIvarRefExpr *IV, SourceLocation OrigStart,
                                 bool &replaced);
    Stmt *RewriteObjCToDNestedIvarRefExpr(Stmt *S, bool &replaced);
    ...
    Stmt *RewriteObjCToDStringLiteral(ObjCStringLiteral *Exp);
    Stmt *RewriteObjCToDProtocolExpr(ObjCProtocolExpr *Exp);
    void WarnAboutReturnGotoStmts(Stmt *S);
    void HasReturnStmts(Stmt *S, bool &hasReturns);
    void RewriteTryReturnStmts(Stmt *S);
    void RewriteSyncReturnStmts(Stmt *S, std::string buf);
    Stmt *RewriteObjCToDTryStmt(ObjCAtTryStmt *S);
    Stmt *RewriteObjCToDSynchronizedStmt(ObjCAtSynchronizedStmt *S);
    Stmt *RewriteObjCToDThrowStmt(ObjCAtThrowStmt *S);
    Stmt *RewriteObjCToDForCollectionStmt(ObjCForCollectionStmt *S,
    ...
    // Metadata emission.
    void RewriteObjCToDClassMetaData(ObjCImplementationDecl *IDecl,
                                  std::string &Result);

    void RewriteObjCToDCategoryImplDecl(ObjCCategoryImplDecl *CDecl,
                                     std::string &Result);

    template<typename MethodIterator>
    void RewriteObjCToDMethodsMetaData(MethodIterator MethodBegin,
    ...
    void RewriteObjCToDProtocolMetaData(ObjCProtocolDecl *Protocol,
                                     llvm::StringRef prefix,
                                     llvm::StringRef ClassName,
                                     std::string &Result);
    void RewriteObjCToDProtocolListMetaData(const ObjCList<ObjCProtocolDecl> &Prots,
                                         llvm::StringRef prefix,
                                         llvm::StringRef ClassName,
                                         std::string &Result);
  ...
  CStyleCastExpr* NoTypeInfoCStyleCastExpr(ASTContext *Ctx, QualType Ty,
	...
	bool isDKeyword (std::string str)
	{
...
std::string RewriteObjCToD::rewriteIdentifier (std::string str)
{
	if (isDKeyword(str)) return str + '_';
	return str;
}

std::string RewriteObjCToD::rewriteType (std::string str)
{
...
std::string RewriteObjCToD::rewriteType (const QualType& type, bool rewriteIdToObject)
{		
...
std::string RewriteObjCToD::rewriteFunctionPointerType (const QualType& type)
{
	if (type->isFunctionPointerType() || type->isBlockPointerType())
	{			
		...
		if (const FunctionProtoType* fpType = pointeeType->getAs<FunctionProtoType>())
		{
			...
			std::string code = rewriteType(fpType->getResultType(), false) + " function (";
			unsigned numberOfArgs = fpType->getNumArgs();
			size_t i = 0;
			
			for (FunctionProtoType::arg_type_iterator iter = fpType->arg_type_begin(),
				end = fpType->arg_type_end(); iter && (iter != end); iter++)
			{
				code += rewriteType(*iter, false);
...
std::string RewriteObjCToD::rewriteObjCObjectPointerType (const QualType& type)
{
	if (type->isObjCObjectPointerType() && !type->isObjCBuiltinType())
	{
...
void RewriteObjCToD::RewriteBlocksInFunctionProtoType(QualType funcType,
                                                   NamedDecl *D) {
...
void RewriteObjCToD::CheckFunctionPointerDecl(QualType funcType, NamedDecl *ND) {
...
RewriteObjCToD::RewriteObjCToD(std::string inFile, llvm::raw_ostream* OS,
                         Diagnostic &D, const LangOptions &LOpts,
                         bool silenceMacroWarn)
...
ASTConsumer *clang::CreateObjCToDRewriter(const std::string& InFile,
                                       llvm::raw_ostream* OS,
                                       Diagnostic &Diags,
                                       const LangOptions &LOpts,
                                       bool SilenceRewriteMacroWarning) {
  return new RewriteObjCToD(InFile, OS, Diags, LOpts, SilenceRewriteMacroWarning);
}

void RewriteObjCToD::Initialize(ASTContext &context) {
  Context = &context;
...
void RewriteObjCToD::HandleTopLevelSingleDecl(Decl *D) {
...
static std::vector<llvm::StringRef> splitString (llvm::StringRef str, char c)
{
...
void RewriteObjCToD::RewriteInclude ()
{
	...
	for (const char* bufPtr = mainBufStart; bufPtr < mainBufEnd; ++bufPtr)
	{
		if (*bufPtr == '#')
		{
			const size_t locStartOfLine = bufPtr - mainBufStart;
			size_t length = 1;
			...
			bool isImport = !strncmp(bufPtr, "import", importLen);
			bool isInclude = !strncmp(bufPtr, "include", includeLen);
			
			if (isImport || isInclude)
			{
				// replace #import or #include with import
				length += isImport ? importLen : includeLen;
				SourceLocation importLoc = locStart.getFileLocWithOffset(locStartOfLine);
				ReplaceText(importLoc, length, "import");
				bufPtr += length;
				
				const size_t startOfSpaces = bufPtr - mainBufStart;
				length = 1;
				...
				if (length > 1)
					ReplaceText(locStart.getFileLocWithOffset(startOfSpaces), length, " ");
				...
				llvm::StringRef str(start, end - start);
				...
				size_t inner = 0;
				size_t outer = 0;
				std::vector<llvm::StringRef> strings = splitString(str, '/');
				
				foreach (llvm::StringRef s, strings)
				{
					llvm::StringRef t = s;					
					inner = 0;
				...
				ReplaceText(locStart.getFileLocWithOffset(start - mainBufStart), end - start, buffer);
			}
		}
	}
}
...
void RewriteObjCToD::RewritePropertyImplDecl(ObjCPropertyImplDecl *PID,
                                          ObjCImplementationDecl *IMD,
                                          ObjCCategoryImplDecl *CID) {
  ...
  RewriteObjCToDMethodDecl(PD->getGetterMethodDecl(), Getr);
  Getr += "{ ";
  ...
  RewriteObjCToDMethodDecl(PD->getSetterMethodDecl(), Setr);
  Setr += "{ ";
...
void RewriteObjCToD::RewriteForwardClassDecl(ObjCClassDecl* classDecl)
{
	...
	// Translate to imports with the same name as the class.
	std::string importString;
	
	for (ObjCClassDecl::iterator iter = classDecl->begin(), end = classDecl->end(); iter != end; ++iter)
		importString += "import " + iter->getInterface()->getNameAsString() + ";\n";
		
	// Replace the @class with imports corresponding to the classes.
	ReplaceText(startLoc, semiPtr - startBuf + 1, importString);
}

void RewriteObjCToD::RewriteMethodDeclaration(ObjCMethodDecl* method, size_t length)
{
	// Get the start location and compute the semi location.
	SourceLocation startLoc = method->getLocation().getFileLocWithOffset(length);
	const char *startBuf = SM->getCharacterData(startLoc);
	const char *semiPtr = strchr(startBuf, ';');
	...
	RewriteTypeIntoString(method->getResultType(), code, fpRetType);
	code += ' ';
	
	RewriteTypeIntoString(method->getResultType(), implCode, fpRetType);
	
	std::string selector = method->getSelector().getAsString();
	implCode += ", \"" + selector + "\"";
	
	size_t index = selector.find(':');
	std::string methodName = rewriteIdentifier(selector.substr(0, index));
	code += methodName + " (";
	unsigned numberOfParams = method->param_size();

	if (numberOfParams > 0)
	{
		unsigned i = 0;
		
		for (ObjCMethodDecl::param_iterator iter = method->param_begin(), end = method->param_end(); iter != end; iter++)
		{
			ParmVarDecl* paramDecl = *iter;
			RewriteTypeIntoString(paramDecl->getType(), code, fpRetType);
			code += " " + paramDecl->getNameAsString();
			implCode += ", " + rewriteType(paramDecl->getType());
			args += paramDecl->getNameAsString();
	...
	ReplaceText(startLoc, semiPtr - startBuf + 1, code);
}

void RewriteObjCToD::RewriteProperty(ObjCPropertyDecl *prop) {
...
void RewriteObjCToD::RewriteCategoryDecl(ObjCCategoryDecl *CatDecl) {
...
void RewriteObjCToD::RewriteProtocolDecl (ObjCProtocolDecl* protocol)
{
#if 0
	// Get the start location and compute the semi location.
	SourceLocation startLoc = protocol->getLocation();
	const char* startBuf = SM->getCharacterData(startLoc);
	const char* semiPtr = strchr(startBuf, ';');
	
	std::string code;
	
	code = "interface I" + protocol->getDeclName().getAsString() + "\n{\n";
	ReplaceText(startLoc, semiPtr - startBuf + 1, code);
	
	ObjCProtocolDecl::instmeth_iterator it = protocol->instmeth_begin();
	RewriteMethodDeclaration(*it);
	it++;
		
	for (ObjCProtocolDecl::instmeth_iterator e = protocol->instmeth_end(); it != e; it++)	
		RewriteMethodDeclaration(*it);
		
	for (ObjCProtocolDecl::classmeth_iterator i = protocol->classmeth_begin(), e = protocol->classmeth_end(); i != e; i++)	
		RewriteMethodDeclaration(*i);
		
	for (ObjCInterfaceDecl::prop_iterator i = protocol->prop_begin(), e = protocol->prop_end(); i != e; i++)	
		RewriteProperty(*i);
...
#endif
#if 1
	SourceRange range = protocol->getSourceRange();
	...
	ReplaceText(startLoc, endBuf - startBuf + 4, code);
...
void RewriteObjCToD::RewriteForwardProtocolDecl(ObjCForwardProtocolDecl *PDecl) {
...
void RewriteObjCToD::RewriteTypeIntoString(QualType type, std::string& resultStr, const FunctionType*& fpRetType)
{	
	/*if (type->isFunctionPointerType() || type->isBlockPointerType())
	{
		// needs special handling, since pointer-to-functions have special
		// syntax (where a decaration models use).
		QualType retType = type;
		QualType pointeeType;
		...
		if ((fpRetType = pointeeType->getAs<FunctionType>()))
		{
			resultStr += rewriteType(fpRetType->getResultType());
			resultStr += "function (";
		}
	}
	
	else*/
		resultStr += rewriteType(type);
}

void RewriteObjCToD::RewriteObjCToDMethodDecl(ObjCMethodDecl *OMD,
                                        std::string &ResultStr) {
  //fprintf(stderr,"In RewriteObjCToDMethodDecl\n");
  const FunctionType *FPRetType = 0;
  ResultStr += "\nstatic ";
...
void RewriteObjCToD::RewriteImplementationDecl(Decl *OID) {
  ...
  for (ObjCCategoryImplDecl::instmeth_iterator
    ...
    RewriteObjCToDMethodDecl(OMD, ResultStr);
...
void RewriteObjCToD::RewriteInterfaceDecl(ObjCInterfaceDecl *ClassDecl) {
  std::string ResultStr;
...
Stmt *RewriteObjCToD::RewritePropertySetter(BinaryOperator *BinOp, Expr *newStmt,
                                         SourceRange SrcRange) {
...
Stmt *RewriteObjCToD::RewritePropertyGetter(ObjCPropertyRefExpr *PropRefExpr) {
  // Synthesize a ObjCMessageExpr from a ObjCPropertyRefExpr.
  // This allows us to reuse all the fun and games in SynthMessageExpr().
  ObjCMessageExpr *MsgExpr;
  ObjCPropertyDecl *PDecl = PropRefExpr->getProperty();
...
Stmt *RewriteObjCToD::RewriteObjCToDIvarRefExpr(ObjCIvarRefExpr *IV,
  ...
  if (CurMethodDef) {
    if (BaseExpr->getType()->isObjCObjectPointerType()) {
      ...
      assert(iFaceDecl && "RewriteObjCToDIvarRefExpr - iFaceDecl is null");
      // lookup which class implements the instance variable.
      ObjCInterfaceDecl *clsDeclared = 0;
      iFaceDecl->getDecl()->lookupInstanceVariable(D->getIdentifier(),
                                                   clsDeclared);
      assert(clsDeclared && "RewriteObjCToDIvarRefExpr(): Can't find class");
      ...
      RecordDecl *RD = RecordDecl::Create(*Context, TTK_Struct, TUDecl,
                                          SourceLocation(), II);
      assert(RD && "RewriteObjCToDIvarRefExpr(): Can't find RecordDecl");
...
Stmt *RewriteObjCToD::RewriteObjCToDNestedIvarRefExpr(Stmt *S, bool &replaced) {
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
       CI != E; ++CI) {
    if (*CI) {
      Stmt *newStmt = RewriteObjCToDNestedIvarRefExpr(*CI, replaced);
      if (newStmt)
        *CI = newStmt;
    }
  }
  if (ObjCIvarRefExpr *IvarRefExpr = dyn_cast<ObjCIvarRefExpr>(S)) {
    ...
    Stmt *newStmt = RewriteObjCToDIvarRefExpr(IvarRefExpr, OrigStmtRange.getBegin(),
                                           replaced);
    return newStmt;
  } 
...
///
void RewriteObjCToD::SynthCountByEnumWithState(std::string &buf) {
...
///
Stmt *RewriteObjCToD::RewriteBreakStmt(BreakStmt *S) {
...
///
Stmt *RewriteObjCToD::RewriteContinueStmt(ContinueStmt *S) {
  if (Stmts.empty() || !isa<ObjCForCollectionStmt>(Stmts.back()))
    return S;
...
/// RewriteObjCToDForCollectionStmt - Rewriter for ObjC2's foreach statement.
///  It rewrites:
...
///  }
///
Stmt *RewriteObjCToD::RewriteObjCToDForCollectionStmt(ObjCForCollectionStmt *S,
                                                SourceLocation OrigEnd) {
...
/// RewriteObjCToDSynchronizedStmt -
/// This routine rewrites @synchronized(expr) stmt;
/// into:
/// objc_sync_enter(expr);
/// @try stmt @finally { objc_sync_exit(expr); }
///
Stmt *RewriteObjCToD::RewriteObjCToDSynchronizedStmt(ObjCAtSynchronizedStmt *S) {
  // Get the start location and compute the semi location.
  SourceLocation startLoc = S->getLocStart();
  const char *startBuf = SM->getCharacterData(startLoc);
...
void RewriteObjCToD::WarnAboutReturnGotoStmts(Stmt *S)
{
...
void RewriteObjCToD::HasReturnStmts(Stmt *S, bool &hasReturns) 
{  
  // Perform a bottom up traversal of all children.
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
        CI != E; ++CI)
...
void RewriteObjCToD::RewriteTryReturnStmts(Stmt *S) {
 // Perform a bottom up traversal of all children.
 for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
      CI != E; ++CI)
   if (*CI) {
...
void RewriteObjCToD::RewriteSyncReturnStmts(Stmt *S, std::string syncExitBuf) {
  // Perform a bottom up traversal of all children.
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
       CI != E; ++CI)
    if (*CI) {
...
Stmt *RewriteObjCToD::RewriteObjCToDTryStmt(ObjCAtTryStmt *S) {
  // Get the start location and compute the semi location.
  SourceLocation startLoc = S->getLocStart();
  const char *startBuf = SM->getCharacterData(startLoc);
...
// been rewritten! (which implies the SourceLocation's are invalid).
Stmt *RewriteObjCToD::RewriteObjCToDThrowStmt(ObjCAtThrowStmt *S) {
  // Get the start location and compute the semi location.
  SourceLocation startLoc = S->getLocStart();
  const char *startBuf = SM->getCharacterData(startLoc);
...
Stmt *RewriteObjCToD::RewriteAtEncode(ObjCEncodeExpr *Exp) {
...
Stmt *RewriteObjCToD::RewriteAtSelector(ObjCSelectorExpr *Exp) {
...
CallExpr *RewriteObjCToD::SynthesizeCallToFunctionDecl(
...
bool RewriteObjCToD::needToScanForQualifiers(QualType T) {
...
void RewriteObjCToD::RewriteObjCToDQualifiedInterfaceTypes(Expr *E) {
...
void RewriteObjCToD::RewriteObjCToDQualifiedInterfaceTypes(Decl *Dcl) {
  SourceLocation Loc;
  QualType Type;
...
void RewriteObjCToD::RewriteTypeOfDecl(VarDecl *ND) {
...
void RewriteObjCToD::SynthSelGetUidFunctionDecl() {
...
void RewriteObjCToD::RewriteFunctionDecl(FunctionDecl *FD) {
  ...
  RewriteObjCToDQualifiedInterfaceTypes(FD);
}

void RewriteObjCToD::RewriteBlockPointerType(std::string& Str, QualType Type) {
...
void RewriteObjCToD::RewriteBlockPointerTypeVariable(std::string& Str,
                                                  ValueDecl *VD) {
...
void RewriteObjCToD::RewriteBlockLiteralFunctionDecl(FunctionDecl *FD) {
...
void RewriteObjCToD::SynthSuperContructorFunctionDecl() {
...
void RewriteObjCToD::SynthMsgSendFunctionDecl() {
...
void RewriteObjCToD::SynthMsgSendSuperFunctionDecl() {
...
void RewriteObjCToD::SynthMsgSendStretFunctionDecl() {
...
void RewriteObjCToD::SynthMsgSendSuperStretFunctionDecl() {
...
void RewriteObjCToD::SynthMsgSendFpretFunctionDecl() {
...
void RewriteObjCToD::SynthGetClassFunctionDecl() {
...
void RewriteObjCToD::SynthGetSuperClassFunctionDecl() {
...
void RewriteObjCToD::SynthGetMetaClassFunctionDecl() {
...
Stmt *RewriteObjCToD::RewriteObjCToDStringLiteral(ObjCStringLiteral *Exp) {
...
QualType RewriteObjCToD::getSuperStructType() {
  if (!SuperStructDecl) {
...
QualType RewriteObjCToD::getConstantStringStructType() {
...
Stmt *RewriteObjCToD::SynthMessageExpr(ObjCMessageExpr *Exp,
                                    SourceLocation StartLoc,
                                    SourceLocation EndLoc) {
  if (!SelGetUidFunctionDecl)
    SynthSelGetUidFunctionDecl();
...
Stmt *RewriteObjCToD::RewriteMessageExpr(ObjCMessageExpr *Exp) {
...
QualType RewriteObjCToD::getProtocolType() {
  if (!ProtocolTypeDecl) {
...
/// RewriteObjCToDProtocolExpr - Rewrite a protocol expression into
/// a synthesized/forward data reference (to the protocol's metadata).
/// The forward references (and metadata) are generated in
/// RewriteObjCToD::HandleTranslationUnit().
Stmt *RewriteObjCToD::RewriteObjCToDProtocolExpr(ObjCProtocolExpr *Exp) {
...
bool RewriteObjCToD::BufferContainsPPDirectives(const char *startBuf,
                                             const char *endBuf) {
  while (startBuf < endBuf) {
    if (*startBuf == '#') {
      // Skip whitespace.
...
void RewriteObjCToD::SynthesizeObjCInternalStruct(ObjCInterfaceDecl *CDecl,
                                               std::string &Result) {
...
// RewriteObjCToDMethodsMetaData - Rewrite methods metadata for instance or
/// class methods.
template<typename MethodIterator>
void RewriteObjCToD::RewriteObjCToDMethodsMetaData(MethodIterator MethodBegin,
                                             MethodIterator MethodEnd,
                                             bool IsInstanceMethod,
                                             llvm::StringRef prefix,
                                             llvm::StringRef ClassName,
...
/// RewriteObjCToDProtocolMetaData - Rewrite protocols meta-data.
void RewriteObjCToD::
RewriteObjCToDProtocolMetaData(ObjCProtocolDecl *PDecl, llvm::StringRef prefix,
...
void RewriteObjCToD::
RewriteObjCToDProtocolListMetaData(const ObjCList<ObjCProtocolDecl> &Protocols,
                                llvm::StringRef prefix, llvm::StringRef ClassName,
                                std::string &Result) {
  if (Protocols.empty()) return;

  for (unsigned i = 0; i != Protocols.size(); i++)
    RewriteObjCToDProtocolMetaData(Protocols[i], prefix, ClassName, Result);
...
/// RewriteObjCToDCategoryImplDecl - Rewrite metadata for each category
/// implementation.
void RewriteObjCToD::RewriteObjCToDCategoryImplDecl(ObjCCategoryImplDecl *IDecl,
                                              std::string &Result) {
  ...
  RewriteObjCToDMethodsMetaData(InstanceMethods.begin(), InstanceMethods.end(),
                             true, "CATEGORY_", FullCategoryName.c_str(),
                             Result);

  // Build _objc_method_list for class's class methods if needed
  RewriteObjCToDMethodsMetaData(IDecl->classmeth_begin(), IDecl->classmeth_end(),
  ...
  if (CDecl)
    RewriteObjCToDProtocolListMetaData(CDecl->getReferencedProtocols(), "CATEGORY",
...
/// ivar offset.
void RewriteObjCToD::SynthesizeIvarOffsetComputation(ObjCContainerDecl *IDecl,
                                                  ObjCIvarDecl *ivar,
                                                  std::string &Result) {
  if (ivar->isBitField()) {
...
void RewriteObjCToD::RewriteObjCToDClassMetaData(ObjCImplementationDecl *IDecl,
                                           std::string &Result) {
  ...
  // Protocols referenced in class declaration?
  RewriteObjCToDProtocolListMetaData(CDecl->getReferencedProtocols(),
...
void RewriteObjCToD::SynthesizeMetaDataIntoBuffer(std::string &Result) {
  int ClsDefCount = ClassImplementation.size();
  int CatDefCount = CategoryImplementation.size();
  
  // For each implemented class, write out all its meta data.
  for (int i = 0; i < ClsDefCount; i++)
    RewriteObjCToDClassMetaData(ClassImplementation[i], Result);

  // For each implemented category, write out all its meta data.
  for (int i = 0; i < CatDefCount; i++)
    RewriteObjCToDCategoryImplDecl(CategoryImplementation[i], Result);
...
void RewriteObjCToD::RewriteByRefString(std::string &ResultStr, 
...
std::string RewriteObjCToD::SynthesizeBlockFunc(BlockExpr *CE, int i,
...
std::string RewriteObjCToD::SynthesizeBlockHelperFuncs(BlockExpr *CE, int i,
                                                   llvm::StringRef funcName,
                                                   std::string Tag) {
  std::string StructRef = "struct " + Tag;
...
std::string RewriteObjCToD::SynthesizeBlockImpl(BlockExpr *CE, std::string Tag, 
                                             std::string Desc) {
  std::string S = "\nstruct " + Tag;
...
std::string RewriteObjCToD::SynthesizeBlockDescriptor(std::string DescTag, 
...
void RewriteObjCToD::SynthesizeBlockLiterals(SourceLocation FunLocStart,
                                          llvm::StringRef FunName) {
...
void RewriteObjCToD::InsertBlockLiteralsWithinFunction(FunctionDecl *FD) {
  SourceLocation FunLocStart = FD->getTypeSpecStartLoc();
...
void RewriteObjCToD::InsertBlockLiteralsWithinMethod(ObjCMethodDecl *MD) {
...
void RewriteObjCToD::GetBlockDeclRefExprs(Stmt *S) {
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
       CI != E; ++CI)
    if (*CI) {
...
void RewriteObjCToD::GetInnerBlockDeclRefExprs(Stmt *S, 
                llvm::SmallVector<BlockDeclRefExpr *, 8> &InnerBlockDeclRefs,
                llvm::SmallPtrSet<const DeclContext *, 8> &InnerContexts) {
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
       CI != E; ++CI)
...
QualType RewriteObjCToD::convertFunctionTypeOfBlocks(const FunctionType *FT) {
...
Stmt *RewriteObjCToD::SynthesizeBlockCall(CallExpr *Exp, const Expr *BlockExp) {
...
//    };
//}
Stmt *RewriteObjCToD::RewriteBlockDeclRefExpr(Expr *DeclRefExp) {
...
//
Stmt *RewriteObjCToD::RewriteLocalVariableExternalStorage(DeclRefExpr *DRE) {
...
void RewriteObjCToD::RewriteCastExpr(CStyleCastExpr *CE) {
...
void RewriteObjCToD::RewriteBlockPointerFunctionArgs(FunctionDecl *FD) {
...
bool RewriteObjCToD::PointerTypeTakesAnyBlockArguments(QualType QT) {
...
void RewriteObjCToD::GetExtentOfArgList(const char *Name, const char *&LParen,
                                     const char *&RParen) {
...
void RewriteObjCToD::RewriteBlockPointerDecl(NamedDecl *ND) {
...
std::string RewriteObjCToD::SynthesizeByrefCopyDestroyHelper(VarDecl *VD,
                                                          int flag) {
  std::string S;
...
///
///
void RewriteObjCToD::RewriteByRefVar(VarDecl *ND) {
...
void RewriteObjCToD::CollectBlockDeclRefInfo(BlockExpr *Exp) {
...
FunctionDecl *RewriteObjCToD::SynthBlockInitFunctionDecl(llvm::StringRef name) {
  IdentifierInfo *ID = &Context->Idents.get(name);
...
Stmt *RewriteObjCToD::SynthBlockInitExpr(BlockExpr *Exp,
          const llvm::SmallVector<BlockDeclRefExpr *, 8> &InnerBlockDeclRefs) {
...
// we get this right.
void RewriteObjCToD::CollectPropertySetters(Stmt *S) {
  // Perform a bottom up traversal of all children.
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
       CI != E; ++CI)
    if (*CI)
...
Stmt *RewriteObjCToD::RewriteFunctionBodyOrGlobalInitializer(Stmt *S) {
  ...
  for (Stmt::child_iterator CI = S->child_begin(), E = S->child_end();
    ...
    if (*CI) {
      ...
      if (ObjCIvarRefExpr *IvarRefExpr = dyn_cast<ObjCIvarRefExpr>(S)) {
        ...
        bool replaced = false;
        newStmt = RewriteObjCToDNestedIvarRefExpr(S, replaced);
        if (replaced) {
  ...
  if (ObjCStringLiteral *AtString = dyn_cast<ObjCStringLiteral>(S))
    return RewriteObjCToDStringLiteral(AtString);
  ...
  if (ObjCAtTryStmt *StmtTry = dyn_cast<ObjCAtTryStmt>(S))
    return RewriteObjCToDTryStmt(StmtTry);

  if (ObjCAtSynchronizedStmt *StmtTry = dyn_cast<ObjCAtSynchronizedStmt>(S))
    return RewriteObjCToDSynchronizedStmt(StmtTry);

  if (ObjCAtThrowStmt *StmtThrow = dyn_cast<ObjCAtThrowStmt>(S))
    return RewriteObjCToDThrowStmt(StmtThrow);

  if (ObjCProtocolExpr *ProtocolExp = dyn_cast<ObjCProtocolExpr>(S))
    return RewriteObjCToDProtocolExpr(ProtocolExp);

  if (ObjCForCollectionStmt *StmtForCollection =
        dyn_cast<ObjCForCollectionStmt>(S))
    return RewriteObjCToDForCollectionStmt(StmtForCollection,
  ...
  if (DeclStmt *DS = dyn_cast<DeclStmt>(S)) {
    ...
    // This is because RewriteObjCToDForCollectionStmt() does textual rewriting 
    // and it depends on the original text locations/positions.
    if (Stmts.empty() || !isa<ObjCForCollectionStmt>(Stmts.back()))
      RewriteObjCToDQualifiedInterfaceTypes(*DS->decl_begin());
  ...
  if (CStyleCastExpr *CE = dyn_cast<CStyleCastExpr>(S))
    RewriteObjCToDQualifiedInterfaceTypes(CE);
...
void RewriteObjCToD::RewriteRecordBody(RecordDecl *RD) {
...
void RewriteObjCToD::HandleDeclInMainFile(Decl *D) {
  if (FunctionDecl *FD = dyn_cast<FunctionDecl>(D)) {
  ...
  if (ObjCImplementationDecl *CI = dyn_cast<ObjCImplementationDecl>(D))
    ...
    RewriteObjCToDQualifiedInterfaceTypes(VD);
...
void RewriteObjCToD::HandleTranslationUnit(ASTContext &C) {
  if (Diags.hasErrorOccurred())
    return;
