...
void Driver::BuildActions(const ToolChain &TC, const ArgList &Args,
  ...
  if ((FinalPhaseArg = Args.getLastArg(options::OPT_E)) ||
    ...
    // -{fsyntax-only,-analyze,emit-ast,S} only run up to the compiler.
             ...
             (FinalPhaseArg = Args.getLastArg(options::OPT_rewrite_objc_to_d)) ||
...
Action *Driver::ConstructPhaseAction(const ArgList &Args, phases::ID Phase,
  ...
  case phases::Compile: {
    ...
    } else if (Args.hasArg(options::OPT_rewrite_objc_to_d)) {
      return new CompileJobAction(Input, types::TY_RewrittenObjCToD);
